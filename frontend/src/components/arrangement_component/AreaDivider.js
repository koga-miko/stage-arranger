...
class AreaDivider {
  ...
  clearCurDivInfo() {
    this.curDivInfo = {
      ...
      isLooped: false,
      isSelected: false,
    }; // divListは{col:xxx, row:xxx, stt:this.DividedAreaState.xxx} を保持する配列
  }
  ...
  updateGroupId() {
    ...
    for (let row = 0; row < this.objects2D.length; row++) {
      ...
      if (divPoints.length === 1) {
        if (row === 0) {
          isSeqLineFromFront = true;
        }
        for (let col = startCol; col < this.objects2D[row].length; col++) {
          if (
            col > 0 &&
            this.objects2D[row][col - 1].groupId !==
              this.objects2D[row][col].groupId
          ) {
            break;
          } else {
            endCol = col;
        ...
        isSeqLineFromFront = false;
        endCol = Math.min(divPoints[1] - 1, this.objects2D[row].length - 1);
      } else {
        if (isSeqLineFromFront) {
          let preLength = this.objects2D[row - 1].length;
          if (preStartCol <= 2) {
            startCol = preStartCol;
          } else if (preStartCol < preLength / 2 - 1) {
            startCol = preStartCol + 1;
          } else if (preStartCol <= preLength / 2 + 1) {
            startCol = preStartCol + 2;
          } else if (preStartCol < preLength - 2) {
            startCol = preStartCol + 3;
          } else {
            startCol = preStartCol + 4;
          }
          for (let col = startCol; col < this.objects2D[row].length; col++) {
            if (
              col > 0 &&
      ...
      for (let row = 0; row < this.objects2D.length; row++) {
        ...
        if (divPoints.length === 1) {
          ...
          if (divPoints[0] < this.objects2D[row].length) {
            ...
            for (
              let col = divPoints[0] + 1;
              col < this.objects2D[row].length;
              col++
            ) {
              if (this.objects2D[row][col].groupId !== curGroupId) {
                break;
  ...
  makeDrawPosList() {
    ...
    for (let i = 0; i < this.curDivInfo.divList.length; i++) {
      let dividedArea =
        this.dividedAreas2D[this.curDivInfo.divList[i].row][
          this.curDivInfo.divList[i].col
        ];
  ...
  registerDividing(objects2D, callbackFn) {
    ...
    for (let i = 0; i < objects2D.length; i++) {
      let dividedAreas = [];
      for (let j = 0; j < objects2D[i].length + 1; j++) {
        // objects2D[i]の要素数よりも1多く回ることに注意
        ...
        let dividedArea = {
          ...
          //                    r:r,
          iDvP: {
            x: relStartPos.y + centerPos.x,
            y: -relStartPos.x + centerPos.y,
          }, // (startPosの相対x座標をcenterPosを中心に90度反時計回りに回転)
  ...
  selectDivLinesFromRect(x, y, w, h) {
    ...
    for (let i = 0; i < this.drawPosList2D.length; i++) {
      for (let j = 0; j < this.drawPosList2D[i].length; j++) {
        if (
          x <= this.drawPosList2D[i][j].x &&
          this.drawPosList2D[i][j].x < x + w &&
          y <= this.drawPosList2D[i][j].y &&
          this.drawPosList2D[i][j].y < y + h
        ) {
  ...
  selectDivLinesFromPos(x, y, toggle = false) {
    ...
    for (let i = 0; i < this.drawPosList2D.length; i++) {
      for (let j = 0; j < this.drawPosList2D[i].length; j++) {
        if (
          Math.pow(this.drawPosList2D[i][j].x - x, 2) +
            Math.pow(this.drawPosList2D[i][j].y - y, 2) <=
          Math.pow(r, 2)
        ) {
          hittedDivLines.push(i);
          break;
    ...
    hittedDivLines.forEach((Id) => {
      if (toggle === true) {
        this.divInfos[Id].isSelected = this.divInfos[Id].isSelected
          ? false
          : true;
      } else {
        this.divInfos[Id].isSelected = true;
  ...
  getWhichDividedArea(dividedArea, x, y) {
    ...
    let [ofsX, ofsY] = getRotRelPos(
      x,
      y,
      dividedArea.sPs.x,
      dividedArea.sPs.y,
      -Math.atan2(
        dividedArea.ePs.y - dividedArea.sPs.y,
        dividedArea.ePs.x - dividedArea.sPs.x
      )
    ); // startPosに平行になるように(時計回りで)回転させたstartPosからの相対位置の取得
      ...
      dividedAreas.forEach((dividedArea, col) => {
        ...
        if (dividedArea.stt === this.DividedAreaState.InHit) {
          if (whichDividedArea === this.DividedAreaState.InHit) {
            ...
            this.curDivInfo.divList.push({
              col: col,
              row: row,
              stt: this.DividedAreaState.In2OutHit,
            });
          ...
          if (whichDividedArea === this.DividedAreaState.InHit) {
            ...
            this.curDivInfo.divList.push({
              col: col,
              row: row,
              stt: this.DividedAreaState.Out2InHit,
            });
            // console.log(this.curDivInfo.divList[this.curDivInfo.divList.length-1])
            isChanged = true;
          } else if (whichDividedArea === this.DividedAreaState.OutHit) {
          ...
          if (whichDividedArea === this.DividedAreaState.InHit) {
            ...
            if (
              len >= 2 &&
              this.curDivInfo.divList[0].col === col &&
              this.curDivInfo.divList[0].row === row
            ) {
              ...
              len >= 1 &&
              this.curDivInfo.divList[len - 1].col === col &&
              this.curDivInfo.divList[len - 1].row === row
            ) {
    ...
    if (divList.length >= 1) {
      ...
      for (; i < divList.length - 1; i++) {
        if (divList[i].stt === this.DividedAreaState.In2OutHit) {
          if (
            (divList[i].row === divList[i + 1].row &&
              divList[i + 1].stt === this.DividedAreaState.Out2InHit) ||
            (divList[i].row + 1 === divList[i + 1].row &&
              divList[i + 1].stt === this.DividedAreaState.In2OutHit)
          ) {
            continue;
          } else {
            break;
